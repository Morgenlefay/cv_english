...
industry_positions <- base::list(
  ...
  base::list(
    ...
    description = base::c(
      "Sales and marketing campagin analysis",
      "Built and mantaine dashboards using R",
      "Porfolio segmentation and machine learning"
    )
  ),
  base::list(
...
```{r data_science_writings}
data_science_writings <- base::list(
  base::list(
    title = "[UnaImagen.uy](https://www.unaimagen.uy/)",
    loc = "N/A", 
    end = 2020,
    description = base::c(
      "A long with other economits we created a blog where we post data visualizations about Uruguay. Our goal is to make available data that contributes to the public discusion and the national policy debate."
    )
  )
)
```
...
Last update <br> `r stringr::str_to_title(paste0(lubridate::month(lubridate::today(), label = TRUE, abbr = FALSE), ", ", lubridate::year(lubridate::today())))`.
...
I am interested in challenging Data Science and/or Data Analytics positions with a special focus on those positions that will require the development of Machine Learning algorithms and/or the construction of data analysis apps and dashboards. I am very experienced with R and SQL and have worked with Python and web technologies too.

I hold dual Uruguayan and Polish citizenship and I'm willing to relocate to other countries in Latin America or Europe. You can find out more about me in my [web page](https://danielczarnievicz.netlify.app/portfolio/) or my [GitHub](https://github.com/daczarne).
